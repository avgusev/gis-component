import { createSlice } from '@reduxjs/toolkit';

const initialState = {
  filter: '',
  loading: false,
  data: false as boolean,
  selected: undefined as boolean,
  cql: '(1=1)',
  filterField: 'skeleton',
};

export type AuthoritiesState = Readonly<typeof initialState>;

export const skeletonSlice = createSlice({
  name: 'skeleton',
  initialState: initialState as AuthoritiesState,
  reducers: {
    setPresetSkeleton(state, action) {
      return action.payload;
    },
    setSkeletonFilter(state, action) {
      state.filter = action.payload;
    },
    resetSkeleton() {
      return initialState;
    },
    addOrRemoveSkeleton(state, action) {
      const item = action.payload;

      let filter = '';
      state.selected = item;
      if (state.selected) {
        filter = state?.filterField + ` IS NOT NULL`;
      } else {
        filter = '(1=1)';
      }

      console.log(`Skeleton filter = ${filter}`);
      state.cql = filter;
    },
  },
  extraReducers(builder) {
    // builder
    //   .addMatcher(isFulfilled(getRegions), (state, action) => {
    //     state.loading = false;
    //     state.data = action.payload.data.map((item) => {
    //       return { id: item.dimmember_gid, name: item.name };
    //     });
    //   })
    //   .addMatcher(isPending(getRegions), (state) => {
    //     state.loading = true;
    //   })
    //   .addMatcher(isRejected(getRegions), (state, action) => {
    //     state.loading = false;
    //   });
  },
});

export const { setSkeletonFilter, setPresetSkeleton, resetSkeleton, addOrRemoveSkeleton } = skeletonSlice.actions;
export default skeletonSlice.reducer;
