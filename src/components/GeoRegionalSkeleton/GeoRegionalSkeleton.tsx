...
const GeoRegionalSkeleton = ({
  ...
  const disabledMode = () => {
    try {
      ...
      const wmsRegionalSkeletonRegionShadedGroup: any = map
        .getLayers()
        .getArray()
        .filter((item, index, array) => item.get('name') === 'regional-skeleton-wms-regin_shaded')?.[0];

      wmsRegionalSkeletonRegionShadedGroup
        .getLayers()
        .getArray()
        .forEach((layer) => {
          const lname = layer.get('name');
  ...
  const onSubmit = (formData) => {
    ...
    wmsRegionalSkeletonGroup
      ...
      .forEach((layer) => {
        ...
        const params = source.getParams();
        layer.setVisible(false);
        if (lname === 'lyr_location_regions_shaded' && !formData.shadeRoad) {
          layer.setVisible(true);
